...
public class Main {
    ...
    private static final Marker INPUT_STATION_MARKER = MarkerManager.getMarker("INPUT_STATION");
    private static final Marker INPUT_STATION_WRONG_MARKER = MarkerManager.getMarker("INPUT_STATION_WRONG");
    ...
    private static Station takeStation(String message) {
        for (; ; ) {
            ...
            if (station != null) {
                LOGGER.printf(Level.INFO, INPUT_STATION_MARKER, "Станция: " + line);
                return station;
            }
            System.out.println("Станция не найдена :(");
//------------------------------------------------------------------------
            //добавил try catch для теста error
            try {
                if (line.equals("SkillBox")) {
                    throw new Exception("This is  \"SkillBox\"");
                }
            }catch (Exception e) {
                LOGGER.printf(Level.ERROR, e.getMessage());
                continue;
...
//------------------------------------------------------------------------
            LOGGER.printf(Level.INFO, INPUT_STATION_WRONG_MARKER, "Станция: " + line);
        }
    }
